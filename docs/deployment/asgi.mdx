...
mcp.add_middleware(CORSMiddleware, allow_origins=["*"])

# You can also pass additional middleware when creating the ASGI app
extra_middleware = [Middleware(CORSMiddleware, allow_origins=["https://example.com"])]
http_app = mcp.http_app(middleware=extra_middleware)
```
